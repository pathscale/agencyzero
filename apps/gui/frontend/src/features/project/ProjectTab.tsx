...
export function ProjectPanelToggle(props: { visible: boolean; onToggle: () => void }): JSX.Element {
  ...
  return (
    <button
      ...
      aria-label={label()}
      title={label()}
      class="absolute top-1/2 left-full z-20 flex h-9 w-[18px] -translate-y-1/2 items-center justify-center rounded-r-md border border-primary/40 border-l-0 bg-primary/20 text-primary transition-[color,background-color,border-color,transform] duration-200 hover:translate-x-px hover:border-primary/60 hover:bg-primary/30 motion-reduce:transition-none"
    >
      <Icon
        name="chevron-right"
