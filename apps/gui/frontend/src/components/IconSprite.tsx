...
/**
 * The artwork for each icon, inline.
 *
 * These were `<symbol>` definitions in one hidden sprite and a `<use href>`
 * per icon, which is the ordinary way to do this and does not work here.
 * `blitz-dom` parses every inline `<svg>` into its own `usvg::Tree` from that
 * element's `outer_html` alone (`layout/construct.rs`), so an icon's markup
 * reaches the rasteriser as `<svg><use href="#i-check"/></svg>` with the
 * `<symbol>` it names sitting in a different tree. usvg resolves the reference
 * to nothing and paints an empty tree, which is why every icon had a correct
 * box and a correct stroke colour and no artwork in it.
 *
 * Inlining costs the deduplication the sprite bought and is the only shape
 * that survives per-element parsing. Keep the `viewBox` at `0 0 24 24`: it is
 * set on the `<svg>` in `Icon`, and these children are authored against it.
 */
export const ICON_ART: Record<IconName, JSX.Element> = {
  "arrow-up": (
    <>
...
/**
 * Formerly the hidden `<symbol>` sprite every icon referenced.
 *
 * The artwork is inlined per icon now (see {@link ICON_ART}), so there is
 * nothing to mount. Kept as a no-op because several tests render it at the
 * root the way `App` used to, and removing the export would fail them for a
