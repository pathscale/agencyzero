...
function createWorkspace() {
  ...
  function upsertItem(item: ProjectItem): void {
    setState((d) => {
      let list = d.items[item.projectId];
      if (!list) {
        list = [];
  ...
  async function subscribe(backend: AgencyZeroApi): Promise<void> {
    ...
    await bind("message:appended", (message) => {
      ...
      if (isNew && message.author === "agent" && message.stop !== "continued") {
        setState((d) => {
          d.turnCounts[message.projectId] = ((count = 0) => count + 1)(
            d.turnCounts[message.projectId],
          );
        });
      }
    });
      ...
      if (phase !== "finished") {
        ...
        touchRunStatus(
            ...
            : "compacting — the session is busy, please wait",
          {
            agent,
            model:
              state.tabs.find((tab) => tab.projectId === projectId && tab.agent === agent)?.model ??
              "",
            permission:
