...
// Generated by `build.rs` from the GUI's schema with the `queries` blocks
// stripped: this crate is pinned to the v2-era DSL, which cannot parse the
// query syntax the GUI has since adopted. Columns, and so the on-disk layout,
// are the GUI's verbatim.
#[path = "."]
mod schema {
    macro_rules! table {
        ($module:ident) => {
